...
class MapTools:
    ...
    def do_fwd_KS(self, kappa_l):
        ...
        F_gamma_1 = self.pixel_window * (self.ell_x**2 - self.ell_y**2) * kappa_l_complex / (self.ell**2 + EPS)
        F_gamma_2 = self.pixel_window * 2. * self.ell_x * self.ell_y    * kappa_l_complex / (self.ell**2 + EPS)
    ...
    def do_KS_inversion(self, eps, EB_decompose=False):        
        ...
        if(EB_decompose):
            kappa_KS_E = jnp.fft.ifftn(kappa_ell).real /  self.PIX_AREA
            kappa_KS_B = jnp.fft.ifftn(kappa_ell).imag /  self.PIX_AREA
            return kappa_KS_E, kappa_KS_B
        else:
    ...
    def fourier2map(self, Fx, conjugate=False):
        ...
        Fx_complex = self.pixel_window * (Fx[0] + J * Fx[1])
        if(conjugate):
