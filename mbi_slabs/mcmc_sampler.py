...
class MCMCSampler:
    def __init__(self, transform, Fourier, obs_calc, nz_src_list, nz_lens_list, sigma_noise, nbar):
        self.transform = transform
        self.N_slabs = transform.N_slabs
        self.N_grid = transform.N_grid
        self.Fourier = Fourier
        self.obs_calc = obs_calc
        self.nz_src_list  = nz_src_list
        self.nz_lens_list = nz_lens_list
        self.N_SRC_BINS  = len(self.nz_src_list)
        self.N_LENS_BINS = len(self.nz_lens_list)
    ...
    def setup_model(self, shape_data, counts_data, key):
        ...
        def density_slab_model(prior):            
            if(self.burn_in):
                theta_cosmo = numpyro.deterministic('theta_cosmo', self.theta_fid)
                Dz_src  = numpyro.deterministic('Dz_src', np.zeros(self.N_SRC_BINS))
                m       = numpyro.deterministic('m', np.zeros(self.N_SRC_BINS))
                A_ia    = numpyro.deterministic('A_ia', 0.5)
                eta_ia  = numpyro.deterministic('eta_ia', 0.)
                bta     = numpyro.deterministic('bta', 0.)
                Dz_lens = numpyro.deterministic('Dz_lens', np.zeros(self.N_LENS_BINS))
                bg      = numpyro.deterministic('bg', np.ones(self.N_LENS_BINS))
            else:
                # Sample parameters
            ...
            gamma_k    = get_gamma_from_slabs(self.nz_src_list, Dz_src, theta_cosmo[0], s_slabs)
            gamma_ia   = get_gamma_ia_from_slabs(self.nz_src_list, Dz_src, A_ia, eta_ia, bta, theta_cosmo[0], dens_slabs, s_slabs)
    ...
    def run_mcmc(self, model, sampling_params, prior, rng_key, 
